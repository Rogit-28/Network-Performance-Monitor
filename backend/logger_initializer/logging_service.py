...
class JSONLogger:
    """
    An enhanced JSON-based logging system for network performance metrics with user-configurable data logging.
    """
    
    def __init__(self, log_file: str = "network_metrics.log", log_level: str = "INFO", sync_mode: bool = False, 
                 log_interval: float = 1.0, max_log_size: int = 10*1024*1024, retention_days: int = 7):
        """
        Initialize the enhanced JSON logger.
        ...
        self.logger = logging.getLogger(f"JSONLogger_{id(self)}")  # Use unique logger name to avoid conflicts
        self.logger.setLevel(self.log_level)
        self.logger.addHandler(self.file_handler)
        self.logger.addHandler(console_handler)
        self.logger.propagate = False # Prevent duplicate logs if there's a root logger
        
        # Track which metrics to log based on user preferences
        self.metrics_to_log = {
            'interfaces': True,
            'performance': True,
            'hardware': True,
    ...
    def export_logs(self, export_format: str = "json", start_time: datetime = None, end_time: datetime = None, search_term: str = None):
        """
        Export logs in specified format (JSON or CSV) with optional filtering.
        ...
        Returns:
            List of filtered log entries or None if error occurs
        """
        try:
            ...
            with tempfile.NamedTemporaryFile(mode='w+', delete=False, suffix='.log') as temp_file:
                ...
                try:
                    with open(self.log_file, 'r', encoding='utf-8') as original_file:
                ...
                with open(temp_filename, 'r', encoding='utf-8') as f:
                    ...
                    for line in lines:
                        ...
                        try:
                            ...
                            log_time = datetime.fromisoformat(log_entry['timestamp'].replace('Z', '+00:00'))
                            if stats["start_time"] is None or log_time < stats["start_time"]:
                                stats["start_time"] = log_time
                            if stats["end_time"] is None or log_time > stats["end_time"]:
                            ...
                            # Count metric types
                            metric_type = log_entry.get("metric_type", "UNKNOWN")
    ...
    def configure_logging(self, metrics_config: Dict[str, bool]):
        """
        Configure which metrics to log based on user preferences.
        
        Args:
            metrics_config: Dictionary mapping metric names to boolean values indicating whether to log them
        """
        for metric, should_log in metrics_config.items():
            if metric in self.metrics_to_log:
                self.metrics_to_log[metric] = should_log
    ...
    def log_metric(self, metric_type: str, data: Dict, level: str = "INFO", metric_category: str = "general"):
        """
        Log a metric in enhanced JSON format.
        ...
        """
        if not self.metrics_to_log.get(metric_type, True):
            return  # Skip logging if this metric type is disabled
    ...
    def log_interfaces(self, interfaces: Dict):
        ...
        if self.metrics_to_log.get('interfaces', True):
            enhanced_interfaces = {
                "interfaces": []
            }
    ...
    def log_performance_metrics(self, metrics: List[Dict]):
        ...
        if self.metrics_to_log.get('performance', True):
    ...
    def log_hardware_info(self, hardware_info: List[Dict]):
        ...
        if self.metrics_to_log.get('hardware', True):
